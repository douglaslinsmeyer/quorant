...
INSERT INTO users (id, idp_user_id, email, display_name, is_active) VALUES
    ...
    ('00000000-0000-0000-0000-000000000041', 'zitadel-homeowner-002', 'dave@example.com', 'Dave Wilson (Homeowner)', TRUE) ON CONFLICT DO NOTHING;
...
INSERT INTO organizations (id, type, name, slug, path, city, state, email, phone) VALUES
    ('10000000-0000-0000-0000-000000000001', 'firm', 'Acme Property Management', 'acme-property-mgmt', 'acme_property_mgmt', 'Phoenix', 'AZ', 'info@acmeprops.com', '602-555-0100') ON CONFLICT DO NOTHING;

-- HOA 1: managed by Acme
INSERT INTO organizations (id, type, name, slug, path, city, state, email, phone) VALUES
    ('20000000-0000-0000-0000-000000000001', 'hoa', 'Lakewood Master HOA', 'lakewood-master-hoa', 'lakewood_master_hoa', 'Phoenix', 'AZ', 'board@lakewood.org', '602-555-0200') ON CONFLICT DO NOTHING;

-- HOA 2: self-managed
INSERT INTO organizations (id, type, name, slug, path, city, state, email, phone) VALUES
    ('20000000-0000-0000-0000-000000000002', 'hoa', 'Sunset Villas HOA', 'sunset-villas-hoa', 'sunset_villas_hoa', 'Scottsdale', 'AZ', 'board@sunsetvillas.org', '480-555-0300') ON CONFLICT DO NOTHING;

-- Link firm to HOA 1
INSERT INTO organizations_management (firm_org_id, hoa_org_id) VALUES
    ('10000000-0000-0000-0000-000000000001', '20000000-0000-0000-0000-000000000001') ON CONFLICT DO NOTHING;
...
INSERT INTO memberships (user_id, org_id, role_id, status, joined_at) VALUES
    ...
    ('00000000-0000-0000-0000-000000000003', '10000000-0000-0000-0000-000000000001', (SELECT id FROM roles WHERE name = 'platform_finance'), 'active', now()) ON CONFLICT DO NOTHING;
...
INSERT INTO memberships (user_id, org_id, role_id, status, joined_at) VALUES
    ...
    ('00000000-0000-0000-0000-000000000012', '10000000-0000-0000-0000-000000000001', (SELECT id FROM roles WHERE name = 'firm_support'), 'active', now()) ON CONFLICT DO NOTHING;
...
INSERT INTO memberships (user_id, org_id, role_id, status, joined_at) VALUES
    ...
    ('00000000-0000-0000-0000-000000000041', '20000000-0000-0000-0000-000000000001', (SELECT id FROM roles WHERE name = 'homeowner'), 'active', now()) ON CONFLICT DO NOTHING;
...
INSERT INTO memberships (user_id, org_id, role_id, status, joined_at) VALUES
    ...
    ('00000000-0000-0000-0000-000000000030', '20000000-0000-0000-0000-000000000002', (SELECT id FROM roles WHERE name = 'hoa_manager'), 'active', now()) ON CONFLICT DO NOTHING;
...
INSERT INTO units (id, org_id, label, unit_type, status, voting_weight) VALUES
    ...
    ('30000000-0000-0000-0000-000000000010', '20000000-0000-0000-0000-000000000002', 'Lot 5', 'single_family', 'vacant', 1.00) ON CONFLICT DO NOTHING;
...
INSERT INTO unit_memberships (unit_id, user_id, relationship, is_voter) VALUES
    ...
    ('30000000-0000-0000-0000-000000000002', '00000000-0000-0000-0000-000000000041', 'owner', TRUE) ON CONFLICT DO NOTHING;
...
INSERT INTO funds (org_id, name, fund_type, balance_cents, is_default) VALUES
    ...
    ('20000000-0000-0000-0000-000000000002', 'Reserve Fund', 'reserve', 8000000, FALSE) ON CONFLICT DO NOTHING;
...
INSERT INTO vendors (id, name, contact_email, contact_phone, service_types) VALUES
    ('40000000-0000-0000-0000-000000000001', 'Green Landscapes LLC', 'billing@greenscapes.com', '602-555-9000', '{landscaping,irrigation}') ON CONFLICT DO NOTHING;

INSERT INTO vendor_assignments (vendor_id, org_id, service_scope) VALUES
    ('40000000-0000-0000-0000-000000000001', '20000000-0000-0000-0000-000000000001', 'landscaping') ON CONFLICT DO NOTHING;
...
INSERT INTO amenities (org_id, name, amenity_type, description, is_reservable, capacity, status) VALUES
    ...
    ('20000000-0000-0000-0000-000000000001', 'Clubhouse', 'clubhouse', 'Available for private events', TRUE, 100, 'open') ON CONFLICT DO NOTHING;
...
INSERT INTO plans (id, name, description, plan_type, price_cents, is_active) VALUES
    ...
    ('50000000-0000-0000-0000-000000000003', 'Firm Bundle', 'Management firm with bundled HOA coverage', 'firm_bundle', 49900, TRUE) ON CONFLICT DO NOTHING;
...
INSERT INTO entitlements (plan_id, feature_key, limit_type, limit_value) VALUES
    ...
    ('50000000-0000-0000-0000-000000000003', 'webhooks.enabled', 'boolean', NULL) ON CONFLICT DO NOTHING;
...
INSERT INTO org_subscriptions (org_id, plan_id, status) VALUES
    ...
    ('20000000-0000-0000-0000-000000000002', '50000000-0000-0000-0000-000000000001', 'active') ON CONFLICT DO NOTHING;
