...
func newTestService() (*fin.FinService, *mockAssessmentRepo, *mockPaymentRepo, *mockBudgetRepo, *mockFundRepo, *mockCollectionRepo) {
	...
	svc := fin.NewFinService(assessments, payments, budgets, funds, collections, nil, audit.NewNoopAuditor(), queue.NewInMemoryPublisher(), ai.NewNoopPolicyResolver(), logger)
...
func TestCreateAssessment_PostsJournalEntry(t *testing.T) {
	...
	svc := fin.NewFinService(assessments, payments, budgets, funds, collections, glService, audit.NewNoopAuditor(), queue.NewInMemoryPublisher(), ai.NewNoopPolicyResolver(), logger)
